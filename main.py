...
@app.route('/')
def home():
    return "AI 戰情室大腦運轉中！(搭載高階防呆與基本面模組)"
...
def predict():
    ...
    try:
        ...
        try:
            ...
            print(f"[{symbol}] Yahoo 基本面資料缺失或解析失敗，已略過: {info_err}")
        # ==========================================
        ...
        if interval == '1d':
            try:
                ...
                if isinstance(df_chips, pd.DataFrame) and not df_chips.empty:
                    ...
                    chip_info = pivot_df.tail(20).to_string() 
        ...
        try:
            ...
            ai_data = {"signal": "解析失敗", "pressure": "--", "support": "--", "stop_loss": "--", "path_up": "--", "path_down": "--", "stars": 0, "advice": ["AI 繁忙", "請重試"]}
